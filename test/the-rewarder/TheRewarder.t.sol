...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        ...
        uint256 numOfDVT = (TOTAL_DVT_DISTRIBUTION_AMOUNT - ALICE_DVT_CLAIM_AMOUNT) / PLAYER_DVT_CLAIM_AMOUNT;
        uint256 numOfWeth = (TOTAL_WETH_DISTRIBUTION_AMOUNT - ALICE_WETH_CLAIM_AMOUNT) / PLAYER_WETH_CLAIM_AMOUNT;

        Claim[] memory claims = new Claim[](numOfDVT + numOfWeth);
        for (uint256 i = 0; i < numOfDVT; i++) {
            claims[i] = Claim({
                batchNumber: 0,
        ...
        for (uint256 i = numOfDVT; i < numOfDVT + numOfWeth; i++) {
            claims[i] = Claim({
                batchNumber: 0,
        ...
        console.log(weth.balanceOf(address(distributor)));
